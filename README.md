...
| Inline block encryption | Блоки ` ```secret ` в любой заметке шифруются при сохранении, расшифровываются при открытии |
...
### Inline-шифрование отдельных блоков

Можно шифровать отдельные блоки внутри **любой** заметки (не обязательно `.secret.md`):

````markdown
# Обычная заметка

Это публичный текст, он остаётся как есть.
...
**Поведение:**
- При сохранении: все ` ```secret ` блоки автоматически шифруются → на диске лежат как ` ```ocke-v1 `
- При открытии (авто-режим): блоки расшифровываются и показываются как ` ```secret ` с plaintext
- При открытии (ручной режим): блоки остаются зашифрованными, расшифровка через "Decrypt selection"
- Если KMS недоступен: блок остаётся зашифрованным, остальная заметка работает нормально
- Каждый блок шифруется независимо (свой DEK)

**Настройка режима:**
- Settings → Cloud KMS Encryption → "Auto-decrypt encrypted blocks on open"
  - ✅ Включено (по умолчанию): блоки расшифровываются автоматически
  - ❌ Выключено: блоки расшифровываются только по запросу через Command Palette
